    ...
    const login = async () => {
      ...
      if (valid) {
        ...
        try {
          ...
          if (e.response.data) { Message({ type: 'error', text: e.response.data.message || '登录失败' }) } else { Message({ text: e, type: 'error' }) }
        }
        // 1. 存储信息
        console.log(data)
          ...
          userMobileLoginMsg(form.mobile)
            .then(() => {
              ...
              const timer = setInterval(() => {
                // disabled.value = true
                // console.log('time decrease', time)
                time.value--
                if (time.value === 0) {
                  clearInterval(timer)
                  time.value = 60
