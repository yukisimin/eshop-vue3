    ...
    <div class="special-list" ref="target">
      <div class="special-item" v-for="item in list" :key="item.id">
        <RouterLink to="/">
          ...
          <div class="meta">
            <p class="title">
              <span class="top ellipsis">{{item.title}}<small>{{item.summary}}</small></span>
              <!-- <span class="sub ellipsis">{{item.subTitle}}</span> -->
            </p>
...
export default {
  ...
  setup () {
    ...
    // const { homeSpecial, result } = useLazyData(findSpecial)
